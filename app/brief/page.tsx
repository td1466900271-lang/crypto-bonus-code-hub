...
export default function Brief(){return <main><header><a className="brand" href="/"><i>₿</i><b>CRYPTO BONUS<span>CODE HUB</span></b></a><nav><a href="/">首页</a><a href="/brief">市场简报</a></nav><a href="/">← 返回首页</a></header><section className="section"><p className="eyebrow">DAILY MARKET BRIEF · 10 JUL 2026</p><h1 style={{fontSize:"56px",maxWidth:"900px",letterSpacing:"-2px"}}>AI 硬件重回修复，<br/>内存链与油价成为主线。</h1><p className="lead">资本重新流向有订单、产能与数据支撑的 AI 瓶颈资产；油价回落为科技估值提供喘息空间。</p><div className="cards">{highlights.map(x=><article key={x.title}><p className="eyebrow">{x.tag}</p><h2 style={{fontSize:"23px"}}>{x.title}</h2><p style={{lineHeight:1.8,color:"#a0a8b2"}}>{x.text}</p></article>)}</div><article style={{marginTop:"36px",padding:"32px",border:"1px solid #544510",borderRadius:"10px",background:"linear-gradient(120deg,#201c0b,#11151a)"}}><p className="eyebrow">TRADER WATCHLIST</p><h2>今天重点观察</h2><div className="steps">{["TSMC 后续营收与关键价位能否确认突破","MU、SNDK 与 DRAM 能否出现第二日放量","原油能否维持弱势，继续缓解估值压力"].map((x,i)=><article key={x}><b>0{i+1}</b><h3>{x}</h3></article>)}</div></article><section style={{marginTop:"72px"}}><p className="eyebrow">BRIEF ARCHIVE</p><h2>历史市场简报</h2><div className="briefArchive">{archive.map((x,i)=><article key={x[0]}><time>{x[0]}</time><div><h3>{x[1]}</h3><p>{x[2]}</p></div><span>{i===0?"最新":"归档"}</span></article>)}</div></section></section><footer><a className="brand" href="/"><i>₿</i><b>CRYPTO BONUS<span>CODE HUB</span></b></a><p>基于公开市场资料重新整理，仅供信息参考，不构成投资建议。</p><span>每日更新</span></footer></main>}
